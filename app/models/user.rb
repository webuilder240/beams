class User < ApplicationRecord
  ...
  after_save :sync_password_credential, if: -> { password.present? }
  ...
  def self.find_or_create_for_oauth(provider:, uid:, email:)
    ...
    if (existing = find_by(email: normalized_email))
      existing.oauth_identities.create!(provider: provider, uid: uid)
      return existing
    end

    return nil unless allowed_oauth_email?(normalized_email)
  ...
  def sync_password_credential
    ...
    pc.password_confirmation = password_confirmation if password_confirmation.present?
    pc.save!
    # save! 後は仮想属性をクリアして二重保存を避ける
    self.password = nil
