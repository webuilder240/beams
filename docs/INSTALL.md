...
Beams は **買い切り・自社サーバー設置型**の BI ツールとして、単一 Docker コンテナで配布する（ONCE 形態）。本書はまっさらな Linux サーバーへの設置から、TLS・バックアップ・自動アップデート・ロールバックまでの運用手順をまとめる。
...
TLS_DOMAIN=beams.example.com \
sudo -E bash deploy/once/install.sh
```
...
3. ホスト env ファイル `/etc/beams/beams.env`（権限 `600`）に `RAILS_MASTER_KEY` / `TLS_DOMAIN` / `IMAGE` を書き出し
4. named volume `beams_storage` を作成（冪等）
5. `docker pull "$IMAGE"`
6. 既存 `beams` コンテナがあれば停止・削除し、`docker run -d` で再起動
   （`--name beams --restart unless-stopped -p 80:80 -p 443:443 -v beams_storage:/rails/storage --env-file /etc/beams/beams.env "$IMAGE"`）
...
| `TLS_DOMAIN` | 任意 | 設定すると Thruster が Let's Encrypt で当該ドメインの証明書を取得し HTTPS(443) 終端する。未設定なら HTTP(80) のみ。 |
...
待受ポートは `TLS_DOMAIN` の有無で変わる。

| `TLS_DOMAIN` | 待受 | 挙動 |
|--------------|------|------|
| 未設定 | HTTP **80** のみ | TLS 終端なし。リバースプロキシ無しで HTTP 直アクセス。 |
| 設定あり | HTTP **80** + HTTPS **443** | Thruster が Let's Encrypt で証明書を自動取得し 443 で HTTPS 終端、80→443 へリダイレクト。 |

コンテナは常に 80/443 の両方を公開する（`-p 80:80 -p 443:443`、Dockerfile も `EXPOSE 80` / `EXPOSE 443`）。実際に 443 を使うかは `TLS_DOMAIN` の有無で Thruster が決める。

アプリ側は `TLS_DOMAIN` 設定時のみ `config.assume_ssl` / `config.force_ssl` を有効化し、`/up`（ヘルスチェック）は HTTPS リダイレクトから除外する（`config/environments/production.rb` が `Beams::Once::TlsConfig` で判定）。

### Thruster 関連 env と既定値

| env | 既定 | 説明 |
|-----|:---:|------|
| `TLS_DOMAIN` | （未設定） | 設定時のみ HTTPS(443) 終端を有効化。 |
| `HTTP_PORT` | `80` | Thruster の HTTP 待受ポート。 |
| `HTTPS_PORT` | `443` | Thruster の HTTPS 待受ポート（`TLS_DOMAIN` 設定時に使用）。 |
| `TARGET_PORT` | `3000` | Thruster が転送する先（Puma）のポート。 |

通常は既定のままでよい。`install.sh` はポートを `-p 80:80 -p 443:443` で固定マッピングしているため、`HTTP_PORT` / `HTTPS_PORT` を変更する場合はコンテナの公開ポートも合わせて調整すること。
...
# HTTPS（TLS_DOMAIN 設定時）
curl -fsS https://beams.example.com/up
```
