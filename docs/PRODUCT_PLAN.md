...
- **`web` は Thruster（`thrust`）で Puma をラップ**。Thruster が HTTP/2・port 80 待受・静的アセット配信・X-Sendfile（CSVダウンロード）を担当する。TLS 終端は ONCE プラットフォーム側に委ねるため、Beams コンテナ自体は HTTP 80 のみ公開する。
