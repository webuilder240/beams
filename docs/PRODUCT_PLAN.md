...
配布レイヤは Kamal を廃し、純シェルのインストーラ（`deploy/once/install.sh`）で `docker pull` + `docker run`（named volume `beams_storage` ＝ `/rails/storage` 永続化、`--env-file /etc/beams/beams.env` で鍵受け渡し）する。自動アップデートは systemd timer（`deploy/once/once-update.{service,timer}` → `bin/once-update`）が daily に実行し、更新前にバックアップ（失敗時は中止）、イメージ差分時のみコンテナを再生成する。TLS は `TLS_DOMAIN` 設定時に Thruster が Let's Encrypt で 443 終端する（未設定なら HTTP 80 のみ）。手順は [docs/INSTALL.md](INSTALL.md)。
