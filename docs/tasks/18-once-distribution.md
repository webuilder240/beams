...
> **注（2026-06-06）**: 本トピックで導入した自前配布層（自前インストーラ・自前 systemd 自動アップデート層・自前 TLS 設定など、グループ B〜D の `deploy/...` 配下・`bin/...-update`・`lib/beams/once/{updater,tls_config}.rb` 等）は[トピック26](26-once-platform.md)で `basecamp/once` 採用により全撤去された。本ファイルは履歴として完了状態のまま据え置く（個別チェックボックスはマージ済み PR の記録として保持）。
